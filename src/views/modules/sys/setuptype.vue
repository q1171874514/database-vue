    ...
    <el-card shadow="never" class="aui-card--fill">
        <div class="mod-sys__setuptype}">
            ...
            <el-table v-loading="dataListLoading" :data="dataList" border @selection-change="dataListSelectionChangeHandle" style="width: 100%;">
                ...
                <el-table-column prop="typeName" label="类名" header-align="center" align="center">
                    <template slot-scope="scope">
                        <el-button type="text" @click="childHandle(scope.row)">{{ scope.row.typeName }}</el-button>
                    </template>
                </el-table-column>
...
<script>
    ...
    import AddOrUpdate from './setuptype-add-or-update'
    ...
    export default {
        ...
        data() {
            return {
                mixinViewModuleOptions: {
                    getDataListURL: '/sys/setup/type/page',
                    getDataListIsPage: true,
                    exportURL: '/sys/setup/type/export',
                    deleteURL: '/sys/setup/type',
                    deleteIsBatch: true
                },
                dataForm: {
                    id: ''
        ...
        methods: {
            // 子级
            childHandle(row) {
                console.log(`${this.$route.name}__${row.id}`)
                console.log(`${this.$route.meta.title} - ${row.typeName}`)
                console.log(routeParams, this.$router)
                // 路由参数
                const routeParams = {
                    routeName: `${this.$route.name}__${row.id}`,
                    title: `${this.$route.meta.title} - ${row.typeName}`,
                    path: 'sys/setupdata',
                    params: {
                        setUpTypeId: row.id
                    }
